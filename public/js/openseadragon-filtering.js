...
(function() {
    ...
    $.Viewer.prototype.setFilterOptions = function(options) {
        console.log('Setting viewer filter options');
        if (!this.filterPluginInstance) {
            console.log('making new filter!');
            options = options || {};
            options.viewer = this;
            this.filterPluginInstance = new $.FilterPlugin(options);
        } else {
            console.log('updating old filter');
    ...
    $.FilterPlugin = function(options) {
        console.log('Inside $.FilterPlugin');
        options = options || {};
        if (!options.viewer) {
    ...
    $.Filters = {
        ...
        COLORMAP: function(cmap, center) {
            var resampled_cmap = cmap.slice(0);
            var diff = 255 - center;
            for(var i = 0; i < 256; i++) {
                if(i > center){
                    var offset = i - center;
                ...
                resampled_cmap[i] = cmap[position];
            }
